...
def load_ml_toolkit(file_path=r"Gradio_toolkit"):

 """
    This function loads the ML items into this file. It takes the path to the ML items to load it.

    Args:
        file_path (regexp, optional): It receives the file path to the ML items, which is Gradio_toolkit.
...
loaded_toolkit = load_ml_toolkit(r"Gradio_toolkit") 
...
def process_and_predict(*args, encoder=encoder, scaler=scaler, model=loaded_rf_model):
    """
    ...
    Args:
        ...
        model (random_forest, optional): This is the model that was trained and is to be used for the prediction. Since random_forest seems to have issues with Pickle, import as a standalone. It defaults to "model", as loaded.
    ...
    encoded_categoricals = encoder.transform(input_data[categorical_cols])
    ...
    df_processed.drop(columns=categorical_cols, inplace=True)
...
with gr.Blocks() as turn_on_the_gradio:
    ...
    gr.Markdown("""This app uses a machine learning model to predict whether or not a customer will churn based on inputs made by the user.""")
    ...
    with gr.Accordion(" inputs Explanation"):
...
turn_on_the_gradio.launch(favicon_path=r"Customer Churn.png",
                          inbrowser= True)
